...
import websiteInquiryApiPlugin from './plugins/vite-plugin-website-inquiry-api.js';

import { readFileSync } from 'node:fs';

const pkg = JSON.parse(readFileSync('./package.json', 'utf-8'));
// Firebase v12 has no "." export — pre-bundling it crashes Vite on restart
const OPTIMIZE_DEPS_SKIP = new Set(['firebase']);
const allDeps = Object.keys(pkg.dependencies || {}).filter(
	(dep) => !OPTIMIZE_DEPS_SKIP.has(dep)
);
